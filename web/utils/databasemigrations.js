...
module.exports = function(pool) {
    (async function setupMigrationsTableIfNotExists() { //This function will run automatically as soon as this module is imported
        try {
            const [res] = await query("SELECT * FROM information_schema.tables WHERE table_schema = 'studentnet' AND table_name = 'dbmigrations' LIMIT 1"); //See if we've setup the dbmigrations table yet
